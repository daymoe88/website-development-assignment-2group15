...
if (!empty($_POST['address']) && !empty($_POST['postcode']) && !empty($_POST['cardName']) && !empty($_POST['cardNo']) && !empty($_POST['cvv']))
{
    ...
    // then, check that the date is valid
    ...
    {
        $query = "INSERT INTO Purchase VALUES (".$purchaseNo.", ".$_SESSION['userID'].", ".$item['productID'].", ".date('Y-m-d H:i:s').", 1, ".$item['quantity'].", ?, ?, ?, ?, ?, ?);";

    }

    // then, create a query to insert the purchase details of the first item into the database
    $query = "INSERT INTO Purchase VALUES (null, ".$_SESSION['userID'].", );";
