...
function get_products($conn, $console, $genre)
{
    ...
    else
    {
        if ($genre == 'Other')
        {
            $query = "SELECT * FROM Product WHERE productID NOT IN (SELECT productID FROM ProductGenre WHERE genre='Action' OR genre='Casual' OR genre='Horror' OR genre='Open World');";
            $result = mysqli_query($conn, $query);
            $products = mysqli_fetch_all($result, MYSQLI_ASSOC);
